...
如果旧的 *.log* 文件有记录，则会在 `DBImpl::Recover` 中调用的 `DBImpl::RecoverLogFile` 中读取记录，并在这个时候建立 `MemTable`，并将 log 应用到 `MemTable` 中，并立刻持久化到 *.ldb* 文件中（minor compaction）。

顺带一提的是，其实可以设置 `Option` 中的 `reuse_logs` 选项来选择复用旧的 *.log* 文件，但是前提是 *.log* 是 appendable 的，并且没有经历过 compaction（如果 `MemTable` 中的数据量过大，会在判断 `reuse_logs` 选项之前进行 minor compaction，所以这样就没法复用旧 log 了）。

最后与 *MANIFEST* 文件同理，会通过 `DBImpl::RemoveObsoleteFiles` 清理掉旧的 *.log* 文件。
